...
void LogState::enable_log(const char* component_alias, int64_t log_level) {
  TORCH_INTERNAL_ASSERT(component_alias != nullptr);
  LogStateMap::iterator it = component_alias_to_level.find(component_alias);

  if (it == component_alias_to_level.end()) {
    char* key = new char[std::strlen(component_alias)];
    std::strcpy(key, component_alias);
    component_alias_to_level[key] = log_level;
  } else {
    it->second = log_level;
  }
}

void LogState::enable_log(std::string component_alias, int64_t log_level) {
  enable_log(component_alias.c_str(), log_level);
}

bool LogState::is_log_enabled(const char* component_alias, int64_t log_level) {
  TORCH_INTERNAL_ASSERT(component_alias != nullptr);
  LogStateMap::iterator it = component_alias_to_level.find(component_alias);
  TORCH_CHECK(
      it != component_alias_to_level.end(),
      "Log component alias not found: ",
      component_alias);
  return log_level >= it->second;
}

bool LogState::is_log_enabled(std::string component_alias, int64_t log_level) {
  return is_log_enabled(component_alias.c_str(), log_level);
}

LogState::~LogState() {
  for (auto it = component_alias_to_level.begin();
       it != component_alias_to_level.end();
       it++) {
