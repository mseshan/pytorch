...
class C10_API LogState {
 public:
  ...
  void enable_log(const char* component_alias, int64_t log_level);
  void enable_log(std::string component_alias, int64_t log_level);

  // Check if the given component alias is enabled with a log level less than or
  // equal to the given level
  bool is_log_enabled(const char* component_alias, int64_t log_level);
  bool is_log_enabled(std::string component_alias, int64_t log_level);
 ...
 private:
  ...
  LogStateMap component_alias_to_level;
};
...
#define TORCH_LOG(component_alias, log_level, ...)                        \
  if (::c10::getLogState()->is_log_enabled(component_alias, log_level)) { \
    ::c10::log(::c10::Log(                                                \
        component_alias,                                                  \
        log_level,                                                        \
